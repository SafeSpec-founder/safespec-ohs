...
// Register new user
router.post('/register', (0, validation_1.validateRequest)(validation_1.schemas.register), async (req, res) => {
    try {
...
// Login user
router.post('/login', (0, validation_1.validateRequest)(validation_1.schemas.login), async (req, res) => {
    try {
        ...
        // Verify password
        const isValidPassword = await bcryptjs_1.default.compare(password, userData.hashedPassword);
        if (!isValidPassword) {
            res.status(401).json({
                error: 'Invalid credentials',
                code: 'INVALID_CREDENTIALS'
