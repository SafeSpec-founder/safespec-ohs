...
// Get chat history
router.get('/history', auth_1.authenticateToken, (0, validation_1.validateRequest)(validation_1.schemas.pagination), async (req, res) => {
    try {
...
router.post('/suggestions', auth_1.authenticateToken, async (req, res) => {
    try {
...
function generateContextualSuggestions(context, entityType) {
    const suggestions = [];
    switch (context) {
        case 'dashboard':
            suggestions.push("Show me this month's incident summary", "What are the top safety risks?", "Generate a compliance status report", "Show overdue corrective actions");
            break;
        case 'incidents':
            suggestions.push("How do I create a new incident report?", "Show me incident trends for this quarter", "What are the most common incident types?", "Help me analyze incident severity patterns");
            break;
        case 'audits':
            suggestions.push("Schedule a new safety audit", "Show me upcoming audit deadlines", "Generate an audit findings report", "What are the common audit non-conformities?");
            break;
        case 'corrective_actions':
            suggestions.push("Create a new corrective action", "Show me overdue actions", "Track corrective action effectiveness", "Generate a CAPA status report");
            break;
        default:
            suggestions.push("What can you help me with?", "Show me my safety dashboard", "Generate a monthly safety report", "Help me with compliance tracking");
    }
    return suggestions;
}
// Helper functions for data analysis
async function analyzeIncidentData(db, tenantId, timeRange, filters) {
...
async function analyzeAuditData(db, tenantId, timeRange, filters) {
...
async function analyzeCorrectiveActionData(db, tenantId, timeRange, filters) {
