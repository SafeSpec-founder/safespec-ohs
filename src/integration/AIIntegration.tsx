...
const AIIntegration: React.FC<AIIntegrationProps> = ({ entityType, entityId }) => {
  ...
  const analyzeEntity = async () => {
    ...
    setLoading(true);
    try {
      const analysisResult = await aiService.analyzeEntity(entityType!, entityId!);
  ...
  const generateRecommendations = async () => {
    ...
    setLoading(true);
    try {
      const recommendations = await aiService.generateRecommendations(entityType!, entityId!);
  ...
  if (!result) {
    return (
      <Box sx={{ p: 3 }}>
        <Typography variant="h5" gutterBottom>
          AI Integration - {entityType}: {selectedEntity.name || selectedEntity.title || selectedEntity.id}
        </Typography>
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        ...
        <Button 
          variant="contained" 
          color="primary"
          onClick={result.type === 'analysis' ? generateRecommendations : analyzeEntity}
          aria-label={result.type === 'analysis' ? "Generate recommendations" : "Analyze entity"}
        >
          {result.type === 'analysis' ? 'Generate Recommendations' : 'View Analysis'}
        </Button>
      </Box>
    </Box>
  );
