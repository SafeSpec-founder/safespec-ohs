      ...
      setTimeout(() => {
        resolve({ 
          ...
          endpoints: [
            ...
            { name: 'Incident Management API', status: 'online', latency: '62ms' },
...
const BackendIntegration: React.FC = () => {
  ...
  const handleTestConnection = async () => {
    try {
      ...
      setError('Failed to connect to backend services. Please check your network connection and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Paper elevation={2} sx={{ p: 3, borderRadius: 2 }}>
      ...
      <Typography variant="body2" color="text.secondary" paragraph>
        Test the connection between the UI components and backend services to ensure proper integration.
      </Typography>
      ...
      {apiStatus && (
        <Paper variant="outlined" sx={{ p: 2 }}>
          ...
          <Box sx={{ mt: 3, p: 2, backgroundColor: 'rgba(76, 175, 80, 0.08)', borderRadius: 1 }}>
            <Typography variant="subtitle1" color="success.main" gutterBottom>
              Integration Test Successful
            </Typography>
            <Typography variant="body2">
              All backend services are online and responding correctly. UI components are properly integrated with the backend APIs.
            </Typography>
          </Box>
        </Paper>
      )}
