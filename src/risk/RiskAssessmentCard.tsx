...
import { Typography, Box, Button, TextField, InputAdornment, IconButton } from '@mui/material';
...
const RiskAssessmentCard: React.FC<RiskAssessmentCardProps> = ({ assessmentId, onUpdate }) => {
  ...
  const loadAssessment = async () => {
    ...
    try {
      ...
      if (isOffline) {
        ...
        const cachedAssessment = await riskService.getCachedAssessment(assessmentId);
        if (cachedAssessment) {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      if (isOffline) {
        queueAction('updateAssessmentStatus', { assessmentId: assessment.id, status: newStatus });
      ...
      const updatedAssessment = await riskService.updateAssessmentStatus(assessment.id, newStatus);
      setAssessment(updatedAssessment);
      if (onUpdate) {
        onUpdate(updatedAssessment);
      }
  ...
  return (
    <Box sx={{ 
      ...
      <Typography variant="body2" color="text.secondary" gutterBottom>
        ID: {assessment.id} | Created: {new Date(assessment.createdAt).toLocaleDateString()}
      </Typography>
          ...
          {assessment.controls.map((control: any, index: number) => (
            <li key={index}>
              <Typography variant="body2">
                {control.description} 
                {control.implemented ? 
                  <span style={{ color: '#388e3c', marginLeft: '0.5rem' }}>(Implemented)</span> : 
                  <span style={{ color: '#d32f2f', marginLeft: '0.5rem' }}>(Not Implemented)</span>
                }
              </Typography>
            </li>
          ))}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box>
          ...
          <Box component="span" sx={{ 
            px: 1, 
            py: 0.5, 
            bgcolor: assessment.status === 'open' ? '#f57c00' : 
                    assessment.status === 'in-progress' ? '#1976d2' : 
                    assessment.status === 'mitigated' ? '#388e3c' : 
                    assessment.status === 'accepted' ? '#9c27b0' : '#757575', 
            color: 'white',
            borderRadius: 1,
            fontSize: '0.75rem',
            fontWeight: 'bold'
