...
import { Typography, Box, Button, Grid, Paper, Divider, TextField, InputAdornment } from '@mui/material';
...
const OSHAComplianceDashboard: React.FC = () => {
  ...
  const applyFilters = async (filters: any) => {
    ...
    try {
      ...
      const filteredFindings = await complianceService.getFilteredFindings(filters);
  ...
  const updateFindingStatus = async (findingId: string, newStatus: string) => {
    try {
      if (isOffline) {
        queueAction('updateFindingStatus', { findingId, status: newStatus });
      ...
      await complianceService.updateFindingStatus(findingId, newStatus);
      // Update local state
      const updatedFindings = findings.map(finding => 
        finding.id === findingId ? { ...finding, status: newStatus } : finding
      );
  ...
  const handleExportDashboard = () => {
    ...
    complianceService.exportDashboard(filterOptions)
      .then(url => {
  ...
  const handleSaveFinding = () => {
    ...
    complianceService.updateFinding(selectedFinding.id, selectedFinding)
      .then(() => {
        // Update local state
        const updatedFindings = findings.map(finding => 
  ...
  const calculateRiskLevel = () => {
    ...
    const highSeverityCount = findings.filter(finding => 
  ...
  const getNextInspectionDate = () => {
    ...
    const upcomingInspections = inspections
      .filter(inspection => new Date(inspection.scheduledDate) > new Date())
      .sort((a, b) => new Date(a.scheduledDate).getTime() - new Date(b.scheduledDate).getTime());
    
    if (upcomingInspections.length === 0) return 'Not scheduled';
    
    return new Date(upcomingInspections[0].scheduledDate).toLocaleDateString();
  };
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center', height: '100%' }}>
            ...
            <Typography variant="h3" color={calculateComplianceRate() >= 80 ? 'success.main' : calculateComplianceRate() >= 60 ? 'warning.main' : 'error.main'}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center', height: '100%' }}>
            ...
            <Typography variant="h3" color={findings.filter(f => f.status === 'open').length > 10 ? 'error.main' : findings.filter(f => f.status === 'open').length > 5 ? 'warning.main' : 'success.main'}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center', height: '100%' }}>
            ...
            <Typography variant="h3" color={calculateRiskLevel() === 'High' ? 'error.main' : calculateRiskLevel() === 'Medium' ? 'warning.main' : 'success.main'}>
              {calculateRiskLevel()}
            </Typography>
          </Paper>
        </Grid>
              ...
              >
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                  <Typography variant="body2">
                    Findings: {findings.filter(f => f.locationId === location.id).length}
                  </Typography>
                  <Box sx={{ 
                    px: 1, 
                    py: 0.5, 
                    bgcolor: location.complianceStatus === 'compliant' ? '#4caf50' : 
                            location.complianceStatus === 'non-compliant' ? '#f44336' : 
                            '#ff9800', 
                    color: 'white',
                    borderRadius: 1,
                    fontSize: '0.75rem',
                    ...
                    {location.complianceStatus.toUpperCase()}
                  </Box>
                </Box>
              </Box>
            ))}
              ...
              <Grid container spacing={2} sx={{ mb: 2 }}>
                <Grid item xs={12} sm={4}>
                  <TextField
                    ...
                    onChange={(e) => applyFilters({ ...filterOptions, status: e.target.value })}
                    fullWidth
                    variant="outlined"
                    size="small"
                    SelectProps={{
                ...
                <Grid item xs={12} sm={4}>
                  <TextField
                    ...
                    onChange={(e) => applyFilters({ ...filterOptions, severity: e.target.value })}
                    fullWidth
                    variant="outlined"
                    size="small"
                    SelectProps={{
                ...
                <Grid item xs={12} sm={4}>
                  <TextField
                    ...
                    onChange={(e) => applyFilters({ ...filterOptions, location: e.target.value })}
                    fullWidth
                    variant="outlined"
                    size="small"
                    SelectProps={{
                ...
                <Box 
                  ...
                  sx={{ 
                    ...
                    borderLeft: `4px solid ${
                      finding.severity === 'critical' ? '#d32f2f' : 
                      finding.severity === 'high' ? '#f57c00' : 
                      finding.severity === 'medium' ? '#fbc02d' : 
                      '#388e3c'
                    }`,
                    cursor: 'pointer'
                  }}
                ...
                >
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <Box sx={{ 
                      px: 1, 
                      py: 0.5, 
                      bgcolor: finding.severity === 'critical' ? '#d32f2f' : 
                              finding.severity === 'high' ? '#f57c00' : 
                              finding.severity === 'medium' ? '#fbc02d' : 
                              '#388e3c', 
                      color: 'white',
                  ...
                  <Typography variant="body2" color="text.secondary" gutterBottom>
                    Location: {locations.find(l => l.id === finding.locationId)?.name || 'Unknown'} | 
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      ...
                      <Box component="span" sx={{ 
                        px: 1, 
                        py: 0.5, 
                        bgcolor: finding.status === 'open' ? '#f57c00' : 
                                finding.status === 'in-progress' ? '#2196f3' : 
                                finding.status === 'resolved' ? '#4caf50' : 
                                '#9e9e9e', 
                        color: 'white',
                        borderRadius: 1,
                        fontSize: '0.75rem',
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setSelectedFinding({...selectedFinding, title: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
            ...
            <Grid item xs={12} md={6}>
              ...
              <TextField
                select
                label="Location"
                value={selectedFinding.locationId}
                onChange={(e) => setSelectedFinding({...selectedFinding, locationId: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
              ...
              <TextField
                ...
                type="date"
                value={selectedFinding.identifiedDate ? new Date(selectedFinding.identifiedDate).toISOString().split('T')[0] : ''}
                onChange={(e) => setSelectedFinding({...selectedFinding, identifiedDate: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
                InputLabelProps={{
              ...
              <TextField
                label="Due Date"
                type="date"
                value={selectedFinding.dueDate ? new Date(selectedFinding.dueDate).toISOString().split('T')[0] : ''}
                onChange={(e) => setSelectedFinding({...selectedFinding, dueDate: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
                InputLabelProps={{
              ...
              <TextField
                ...
                onChange={(e) => setSelectedFinding({...selectedFinding, correctiveAction: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
                multiline
