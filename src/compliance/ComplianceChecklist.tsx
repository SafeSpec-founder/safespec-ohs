...
const ComplianceChecklist: React.FC = () => {
  ...
  const [checklists, setChecklists] = useState<ComplianceCategory[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const checklistsData = await complianceService.getChecklists();
      const categoriesData = await complianceService.getCategories();
  ...
  const applyFilters = async (filters: FilterOptions) => {
    ...
    try {
      ...
      const filteredItems = await complianceService.getFilteredItems(selectedCategory.id, filters);
  ...
  const handleExportChecklist = () => {
    // Export the current checklist
    complianceService.exportChecklist(selectedCategory.id)
      .then(url => {
        // Open or download the exported checklist
  ...
  const handlePrintCategory = (categoryId: string) => {
    ...
    complianceService.generatePrintableView(categoryId)
      .then(url => {
        // Open print view
        window.open(url, '_blank');
      })
  ...
  const handleExportCategory = (categoryId: string) => {
    // Export the category items
    complianceService.exportCategory(categoryId)
      .then(url => {
  ...
  const handleSaveItem = () => {
    ...
    complianceService.updateItem(selectedItem.id, selectedItem)
      .then(() => {
        // Update local state
        const updatedItems = items.map(item => 
            ...
            {categories.map(category => (
              <Box 
                ...
                sx={{ 
                  ...
                  borderRadius: 1,
                  bgcolor: selectedCategory?.id === category.id ? '#f5f5f5' : 'transparent',
                  cursor: 'pointer'
                }}
                onClick={() => {
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      onChange={(e) => applyFilters({ ...filterOptions, priority: e.target.value })}
                      fullWidth
                      variant="outlined"
                      size="small"
                      SelectProps={{
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      onChange={(e) => applyFilters({ ...filterOptions, assignee: e.target.value })}
                      fullWidth
                      variant="outlined"
                      size="small"
                      SelectProps={{
                  ...
                  <Box 
                    ...
                    sx={{ 
                      ...
                      borderLeft: `4px solid ${
                        ...
                        item.status === 'non-compliant' ? '#f44336' : 
                  ...
                  >
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      <Box sx={{ 
                        px: 1, 
                        py: 0.5, 
                        bgcolor: item.priority === 'high' ? '#f44336' : 
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Box component="span" sx={{ 
                          px: 1, 
                          py: 0.5, 
                          bgcolor: item.status === 'compliant' ? '#4caf50' : 
                                  item.status === 'non-compliant' ? '#f44336' : 
                                  item.status === 'in-progress' ? '#2196f3' : 
                                  '#9e9e9e', 
                          color: 'white',
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <TextField
                ...
                onChange={(e) => setSelectedItem({...selectedItem, assignee: e.target.value})}
                fullWidth
                margin="normal"
                variant="outlined"
