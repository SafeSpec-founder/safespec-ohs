...
export const fetchCorrectiveActions = createAsyncThunk(
  ...
  async (params: { page?: number; limit?: number; filters?: any }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to fetch corrective actions');
    }
  }
);
...
export const fetchCorrectiveActionById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to fetch corrective action');
    }
  }
);
...
export const createCorrectiveAction = createAsyncThunk(
  ...
  async (correctiveAction: Omit<CorrectiveAction, 'id' | 'createdAt' | 'updatedAt'>, { rejectWithValue }) => {
    try {
      return await correctiveActionService.createCorrectiveAction(correctiveAction);
    } catch (error: any) {
      return rejectWithValue(error.message || 'Failed to create corrective action');
    }
  }
);
...
export const updateCorrectiveAction = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: Partial<CorrectiveAction> }, { rejectWithValue }) => {
    try {
      return await correctiveActionService.updateCorrectiveAction(id, data);
    } catch (error: any) {
      return rejectWithValue(error.message || 'Failed to update corrective action');
    }
  }
);
...
export const deleteCorrectiveAction = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || 'Failed to delete corrective action');
    }
  }
);
...
export const correctiveActionSlice = createSlice({
  ...
  initialState,
  reducers: {
    setCurrentCorrectiveAction: (state, action: PayloadAction<CorrectiveAction | null>) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deleteCorrectiveAction.fulfilled, (state, action) => {
        state.isLoading = false;
        state.correctiveActions = state.correctiveActions.filter(ca => ca.id !== action.payload);
...
// Selectors
export const selectCorrectiveActions = (state: RootState) => state.correctiveActions.correctiveActions;
export const selectCurrentCorrectiveAction = (state: RootState) => state.correctiveActions.currentCorrectiveAction;
export const selectCorrectiveActionsLoading = (state: RootState) => state.correctiveActions.isLoading;
export const selectCorrectiveActionsError = (state: RootState) => state.correctiveActions.error;
export const selectCorrectiveActionsFilters = (state: RootState) => state.correctiveActions.filters;
export const selectCorrectiveActionsPagination = (state: RootState) => state.correctiveActions.pagination;
