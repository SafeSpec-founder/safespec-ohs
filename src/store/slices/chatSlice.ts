...
export const sendMessage = createAsyncThunk(
  'chat/sendMessage',
  async ({ conversationId, content, metadata }: { conversationId: string | null; content: string; metadata?: any }, { rejectWithValue }) => {
    try {
        ...
        if (state.activeConversationId && action.payload.assistantMessage) {
          ...
          if (conversationIndex !== -1) {
            state.conversations[conversationIndex] = {
              ...state.conversations[conversationIndex],
              lastMessage: action.payload.assistantMessage.content.substring(0, 50) + (action.payload.assistantMessage.content.length > 50 ? '...' : ''),
