...
import { Box, Typography, Paper, TextField, Button, FormControl, InputLabel, Select, MenuItem, Grid, CircularProgress, Snackbar, Alert, FormHelperText } from '@mui/material';
import { CorrectiveAction } from '../../store/slices/correctiveActionSlice';
import { useAppDispatch, useAppSelector } from '../../store/index';
import { createCorrectiveAction, updateCorrectiveAction } from '../../store/slices/correctiveActionSlice';
...
const CorrectiveActionForm: React.FC<CorrectiveActionFormProps> = ({
  ...
  const [description, setDescription] = React.useState(correctiveAction?.description || '');
  const [priority, setPriority] = React.useState(correctiveAction?.priority || 'medium');
  const [dueDate, setDueDate] = React.useState<Date | null>(
    correctiveAction?.dueDate ? new Date(correctiveAction.dueDate) : null
  );
  const [assignedTo, setAssignedTo] = React.useState(correctiveAction?.assignedTo || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const correctiveActionData = {
        ...
        relatedIncidentId: correctiveAction?.relatedIncidentId || relatedIncidentId,
      ...
      if (isEditMode && correctiveAction) {
        result = await dispatch(updateCorrectiveAction({ 
          id: correctiveAction.id, 
          data: correctiveActionData 
        })).unwrap();
      } else {
        result = await dispatch(createCorrectiveAction({
          ...correctiveActionData,
          id: uuidv4(), // This would be handled by the backend in a real implementation
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2 }}>
            ...
            {(relatedIncidentId || correctiveAction?.relatedIncidentId) && (
              <Grid item xs={12}>
                <FormControl fullWidth margin="normal" variant="outlined" disabled>
                  <InputLabel id="related-incident-label">Related Incident</InputLabel>
                  <Select
                    labelId="related-incident-label"
                    value={relatedIncidentId || correctiveAction?.relatedIncidentId || ''}
                    label="Related Incident"
                  >
                    <MenuItem value={relatedIncidentId || correctiveAction?.relatedIncidentId || ''}>
                      Incident #{relatedIncidentId || correctiveAction?.relatedIncidentId}
                    </MenuItem>
                  </Select>
                  <FormHelperText>This corrective action is linked to an incident</FormHelperText>
                </FormControl>
              </Grid>
            )}
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            {isEditMode ? 'Corrective action updated successfully!' : 'Corrective action created successfully!'}
          </Alert>
        </Snackbar>
      </Paper>
