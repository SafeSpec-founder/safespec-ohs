...
import { Box, Typography, Paper, Tabs, Tab, Button, Divider, Chip, Grid, IconButton, Tooltip } from '@mui/material';
...
const CorrectiveActionCard: React.FC<CorrectiveActionCardProps> = ({
  ...
  const isOverdue = () => {
    try {
      return new Date(correctiveAction.dueDate) < new Date() && correctiveAction.status !== 'completed';
    } catch (error) {
      return false;
    }
  };
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          {correctiveAction.status !== 'completed' && correctiveAction.status !== 'cancelled' && (
            <Tooltip title="Mark as Complete">
              <IconButton size="small" onClick={() => onComplete?.(correctiveAction)}>
                <AssignmentTurnedInIcon fontSize="small" />
              </IconButton>
            </Tooltip>
          )}
          
          {correctiveAction.status === 'completed' && !correctiveAction.verifiedBy && (
            <Tooltip title="Verify Completion">
              <IconButton size="small" onClick={() => onVerify?.(correctiveAction)}>
          ...
          <Tooltip title="Delete">
            <IconButton size="small" onClick={() => onDelete?.(correctiveAction)}>
      ...
      <Typography variant="h6" gutterBottom noWrap title={correctiveAction.title}>
      ...
      <Box sx={{ mt: 'auto' }}>
        <Grid container spacing={1} sx={{ mb: 2 }}>
          ...
          {correctiveAction.completedDate && (
            <Grid item xs={12}>
              <Typography variant="caption" color="text.secondary">
                Completed: {formatDate(correctiveAction.completedDate)} by {correctiveAction.completedBy}
              </Typography>
            </Grid>
          )}
          ...
          {correctiveAction.verificationDate && (
            <Grid item xs={12}>
              <Typography variant="caption" color="text.secondary">
                Verified: {formatDate(correctiveAction.verificationDate)} by {correctiveAction.verifiedBy}
              </Typography>
            </Grid>
          )}
        </Grid>
