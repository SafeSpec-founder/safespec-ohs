...
import { Box, Typography, Paper, TextField, Button, FormControl, InputLabel, Select, MenuItem, Grid, CircularProgress, Snackbar, Alert } from '@mui/material';
...
const IncidentForm: React.FC<IncidentFormProps> = ({
  ...
  const [date, setDate] = React.useState(incident?.date ? new Date(incident.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]);
  const [severity, setSeverity] = React.useState(incident?.severity || 'medium');
  const [witnesses, setWitnesses] = React.useState(incident?.witnesses?.join(', ') || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const witnessArray = witnesses
        ? witnesses.split(',').map(witness => witness.trim()).filter(Boolean)
        : [];
      ...
      if (isEditMode && incident) {
        result = await dispatch(updateIncident({ 
          id: incident.id, 
          data: incidentData 
        })).unwrap();
      } else {
        result = await dispatch(createIncident({
          ...incidentData,
