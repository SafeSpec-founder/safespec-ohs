...
import { Typography, Box, Button, Grid, Paper, Divider, TextField, Tabs, Tab, Dialog, DialogTitle, DialogContent, DialogActions, FormControl, InputLabel, Select, MenuItem } from '@mui/material';
...
const AuditWorkflow: React.FC = () => {
  ...
  const handleScheduleAudit = async () => {
    try {
      ...
      if (isOffline) {
        queueAction('scheduleAudit', scheduleParams);
  ...
  const handleSelectTemplate = async (template: any) => {
    try {
      ...
      const templateDetails = await auditService.getTemplateDetails(template.id);
  ...
  const handleCompleteAudit = async () => {
    try {
      ...
      if (isOffline) {
        queueAction('completeAudit', { auditId: selectedAudit.id });
        // Update UI optimistically
        setSelectedAudit({
          ...selectedAudit,
          status: 'completed',
      ...
      const updatedAudit = await auditService.completeAudit(selectedAudit.id, completionData);
  ...
  const handleApproveRejectAudit = async () => {
    try {
      ...
      if (isOffline) {
        queueAction(approvalAction === 'approve' ? 'approveAudit' : 'rejectAudit', { 
        ...
        setSelectedAudit({
          ...
          approvedRejectedBy: currentUser?.id
        });
      ...
      if (approvalAction === 'approve') {
        updatedAudit = await auditService.approveAudit(selectedAudit.id, approvalNotes);
      } else {
  ...
  const handleUpdateAuditItem = async (itemId: string, response: string, notes: string) => {
    try {
      if (!selectedAudit) return;
      
      if (isOffline) {
        queueAction('updateAuditItem', { 
          auditId: selectedAudit.id,
          itemId: itemId,
          response: response,
          notes: notes
      ...
      await auditService.updateAuditItem(selectedAudit.id, itemId, { response, notes });
  ...
  const handleAddEvidence = async (itemId: string, evidence: any) => {
    try {
      ...
      const updatedItem = await auditService.updateAuditItem(selectedAudit.id, itemId, {
        evidence: [...(selectedAudit.items.find((item: any) => item.id === itemId)?.evidence || []), {
          ...evidence,
          uploadedAt: new Date().toISOString(),
          uploadedBy: currentUser?.id
        }]
  ...
  const filteredAudits = audits.filter(audit => {
    // Apply status filter
    if (filterOptions.status !== 'all' && audit.status !== filterOptions.status) {
      return false;
    }
    ...
    <Box sx={{ p: 3 }}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(_e, newValue) => {
                      handleFilterChange('type', newValue?.props?.value || 'all');
                    }}
                    fullWidth
                    variant="outlined"
                    size="small"
                ...
                filteredAudits.map((audit) => (
                  <Paper 
                    ...
                    sx={{ 
                      ...
                      mb: 2, 
                      cursor: 'pointer',
                      backgroundColor: selectedAudit?.id === audit.id ? 'action.selected' : 'background.paper',
                      '&:hover': {
                  ...
                  >
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Date: {new Date(audit.scheduledDate || audit.createdAt).toLocaleDateString()}
                    </Typography>
                  </Paper>
                ))
              ) : (
                ...
                templates.map((template) => (
                  <Paper 
                    ...
                    sx={{ 
                      ...
                      mb: 2, 
                      cursor: 'pointer',
                      backgroundColor: selectedTemplate?.id === template.id ? 'action.selected' : 'background.paper',
                      '&:hover': {
                        backgroundColor: 'action.hover'
                      }
                    }}
            ...
            {selectedAudit ? (
              <Box>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  ...
                  <Grid item xs={6}>
                    <Typography variant="body2">
                      <strong>Scheduled Date:</strong> {new Date(selectedAudit.scheduledDate).toLocaleDateString()}
                    </Typography>
                  </Grid>
                </Grid>
                  ...
                  <Paper key={item.id} sx={{ p: 2, mb: 2 }}>
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={6}>
                        <TextField
                          ...
                          label="Response"
                          value={item.response || ''}
                          onChange={(e) => handleUpdateAuditItem(item.id, e.target.value, item.notes || '')}
                          fullWidth
                          variant="outlined"
                          size="small"
                        >
                      ...
                      <Grid item xs={6}>
                        <TextField
                          label="Notes"
                          value={item.notes || ''}
                          onChange={(e) => handleUpdateAuditItem(item.id, item.response || '', e.target.value)}
                          fullWidth
                          variant="outlined"
                          size="small"
                          multiline
                    ...
                    <Box sx={{ mt: 2 }}>
                      <Button 
                        variant="outlined" 
                        size="small" 
                        onClick={() => handleAddFinding(item.id, { 
                          title: 'New Finding',
                          description: '',
                          severity: 'medium',
                          category: 'compliance'
      ...
      <Dialog open={scheduleDialogOpen} onClose={handleCloseScheduleDialog} maxWidth="md" fullWidth>
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) => handleScheduleInputChange('templateId', e.target.value)}
                  label="Template"
                >
                  {templates.map((template) => (
            ...
            <Grid item xs={6}>
              <TextField
                ...
                onChange={(e) => handleScheduleInputChange('assignedTo', e.target.value)}
                fullWidth
                variant="outlined"
              />
            </Grid>
        ...
        <DialogActions>
          ...
          <Button onClick={handleScheduleAudit} variant="contained">Schedule</Button>
        </DialogActions>
      </Dialog>
