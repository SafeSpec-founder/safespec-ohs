...
const SafetyCharts: React.FC = () => {
  ...
  const [lastSuccessfulTimeRange, setLastSuccessfulTimeRange] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDataTimeout = setTimeout(() => {
      ...
      try {
        ...
        if (!validateChartData(incidentData) || 
  ...
  const validateChartData = (chartData: ChartData): boolean => {
    ...
    if (chartData.labels.length === 0 || chartData.data.length === 0) return false;
  ...
  const renderRiskDistributionChart = (): void => {
    ...
    // Validate data before creating chart
    if (!riskData || riskData.length === 0 || riskData.some(item => item === null || item === undefined || isNaN(item))) {
  ...
  const handleDownload = (chartId: string): void => {
    ...
    switch(chartId) {
      ...
      case 'risk':
        ...
        break;
      default:
        addToast({ type: 'error', title: 'Download Error', message: 'Invalid chart selected for download.' });
        return;
    }
    ...
    if (chartRef && chartRef.current) {
      ...
      link.click();
      addToast({ type: 'success', title: 'Download Started', message: `Downloading ${fileName}...` });
    } else {
      addToast({ type: 'error', title: 'Download Error', message: 'Chart canvas not found.' });
    }
  };
  ...
  const handleExportAll = async (): Promise<void> => {
    ...
    setIsExporting(true);
    addToast({ type: 'info', title: 'Exporting PDF', message: 'Generating combined chart report...' });

    // Ensure all charts are rendered and refs are available
    if (!incidentChartRef.current || !complianceChartRef.current || !trainingChartRef.current || !riskDistributionRef.current) {
      addToast({ type: 'error', title: 'Export Error', message: 'One or more charts are not ready for export.' });
    ...
    try {
      ...
      // Add Title
      pdf.setFontSize(18);
      pdf.text('SafeSpec - Safety Charts Report', pageWidth / 2, yPos, { align: 'center' });
      yPos += 10;
      pdf.setFontSize(10);
      ...
      const addChartToPdf = (chartRef: React.RefObject<HTMLCanvasElement>, x: number, y: number) => {
        if (chartRef.current) {
      ...
      // Save the PDF
      pdf.save(`SafeSpec_Charts_Report_${timeRange}_${new Date().toISOString().split('T')[0]}.pdf`);
      addToast({ type: 'success', title: 'Export Successful', message: 'Combined chart PDF generated.' });

    } catch (error) {
      console.error('Error generating PDF:', error);
      addToast({ type: 'error', title: 'Export Failed', message: 'Could not generate the PDF report.' });
    } finally {
      ...
      <div className="chart-grid">
        ...
        <div className="dashboard-card chart-card">
          <div className="card-header">
            ...
            <div className="card-actions">
              <button className="action-button tooltip" onClick={() => handleDownload('incidents')} disabled={loading || isExporting || !dataLoaded}>
        ...
        <div className="dashboard-card chart-card">
          <div className="card-header">
            ...
            <div className="card-actions">
              <button className="action-button tooltip" onClick={() => handleDownload('compliance')} disabled={loading || isExporting || !dataLoaded}>
                <span className="tooltip-content">Download PNG</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
              </button>
            </div>
        ...
        <div className="dashboard-card chart-card">
          <div className="card-header">
            ...
            <div className="card-actions">
              <button className="action-button tooltip" onClick={() => handleDownload('training')} disabled={loading || isExporting || !dataLoaded}>
                <span className="tooltip-content">Download PNG</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
              </button>
            </div>
        ...
        <div className="dashboard-card chart-card">
          <div className="card-header">
            ...
            <div className="card-actions">
              <button className="action-button tooltip" onClick={() => handleDownload('risk')} disabled={loading || isExporting || !dataLoaded}>
                <span className="tooltip-content">Download PNG</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
              </button>
            </div>
