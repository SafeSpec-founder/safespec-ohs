      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab label={`Admins (${users.filter(user => user.role === 'admin').length})`} {...a11yProps(1)} />
            <Tab label={`Managers (${users.filter(user => user.role === 'manager').length})`} {...a11yProps(2)} />
            <Tab label={`Staff (${users.filter(user => user.role === 'staff').length})`} {...a11yProps(3)} />
          </Tabs>
        </Box>
