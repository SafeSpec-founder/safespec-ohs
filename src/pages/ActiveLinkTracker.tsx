...
const ActiveLinkTracker: React.FC<ActiveLinkTrackerProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    links.forEach(link => {
      ...
      const linkPath = href.endsWith('/') && href.length > 1 ? href.slice(0, -1) : href;
      const currentPath = activeLink.endsWith('/') && activeLink.length > 1 ? activeLink.slice(0, -1) : activeLink;
