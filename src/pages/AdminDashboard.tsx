...
const getMetricName = (key: MetricKey): string => {
  switch (key) {
    case 'incidentRate': return 'Incident Rate';
    case 'safetyScore': return 'Safety Score';
    case 'equipmentCompliance': return 'Equipment Compliance';
    case 'trainingCompletion': return 'Training Completion';
    case 'hseTargetProgress': return 'HSE Target Progress';
    case 'nearMisses': return 'Near Misses';
    case 'safetyObservations': return 'Safety Observations';
...
const getMetricSource = (key: MetricKey): string => {
  switch (key) {
    case 'incidentRate': return 'Incident Reports';
    case 'safetyScore': return 'Safety Audits';
    case 'equipmentCompliance': return 'Equipment Inspections';
    case 'trainingCompletion': return 'Training Records';
    case 'hseTargetProgress': return 'Project Goals';
    case 'nearMisses': return 'Incident Reports';
    case 'safetyObservations': return 'Safety Observations';
    default: return 'N/A';
  }
};
...
const formatTrend = (key: MetricKey, trend: number): { text: string; className: string } => {
  const isLowerBetter = key === 'incidentRate' || key === 'nearMisses';
...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log(`AdminDashboard: Fetching metrics for time range: ${timeRange}`);
    setTimeout(() => {
  ...
  return (
    <div className="dashboard-container admin-dashboard">
      ...
      <div className="dashboard-section">
        ...
        {isLoadingPermits ? (
          ...
          <div className="permit-stats-grid">
            {(Object.keys(permitStats) as Array<keyof PermitStats>).map(key => (
              <div key={key} className={`stats-card ${getStatsIconClass(key)}`}>
                <div className="stats-value">{permitStats[key]}</div>
                <div className="stats-label">{key.charAt(0).toUpperCase() + key.slice(1)} Permits</div>
              </div>
            ))}
          </div>
        ) : (
      ...
      <div className="dashboard-section">
        <h2 className="section-title">Performance Metrics Breakdown ({getTimeRangeLabel(timeRange)})</h2>
        <div className="dashboard-card">
          <div className="card-content no-padding">
            {isLoadingMetrics ? (
              <div className="loader-container"><div className="loader"></div><div className="loader-text">Loading metrics...</div></div>
            ) : metrics ? (
                    ...
                    {(Object.keys(metrics) as MetricKey[]).map(key => {
                      ...
                      return (
                        <tr key={key}>
                          ...
                          <td><span className={trendInfo.className}>{trendInfo.text}</span></td>
                          <td>{metric.previousValue !== undefined ? formatMetricValue(key, metric.previousValue) : 'N/A'}</td>
              ...
              <p className="empty-state-row">Could not load performance metrics.</p>
            )}
          </div>
        </div>
      </div>
