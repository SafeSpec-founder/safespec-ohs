...
const PendingApprovals: React.FC = () => {
  ...
  useEffect(() => {
    if (userRole !== 'admin') {
      addToast({ type: 'error', title: 'Access Denied', message: 'You do not have permission to view this page.' });
  ...
  useEffect(() => {
    ...
    const q = query(approvalsCollection, where('status', '==', 'pending'), orderBy('requestedTimestamp', 'desc'));
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      addToast({ type: 'error', title: 'Error', message: 'Could not load pending approvals.' });
      setLoadingApprovals(false);
    });
  ...
  useEffect(() => {
    ...
    const q = query(activityCollection, orderBy('actionedTimestamp', 'desc'), limit(10));
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      addToast({ type: 'error', title: 'Error', message: 'Could not load recent activity.' });
      setLoadingActivity(false);
    });
  ...
  const handleApprovalAction = async (id: string, action: 'approved' | 'rejected'): Promise<void> => {
    if (!typedUser) {
    ...
    if (!approvedItem) {
      addToast({ type: 'error', title: 'Error', message: 'Approval request not found.' });
      return;
    }
    ...
    try {
      ...
      addToast({ type: 'error', title: 'Error', message: `Could not ${action} the request.` });
    }
  };
    ...
    <div className="dashboard-container pending-approvals">
      ...
      <div className="dashboard-card">
        <div className="card-header">
          <div className="card-title">Approval Requests ({loadingApprovals ? '...' : approvals.length})</div>
        </div>
                ...
                <div key={approval.id} className="approval-item">
                  <div className="approval-content">
                    ...
                    <div className="approval-description">
                      <strong>{approval.requestorName}</strong> {approval.requestorEmail && `(${approval.requestorEmail})`} {approval.details}
                    </div>
                    <div className="approval-time">Requested {formatTimestamp(approval.requestedTimestamp)}</div>
                  </div>
        ...
        <div className="card-content no-padding">
          <div className="recent-activity-table-container">
            <table className="data-table">
              ...
              <tbody>
                {loadingActivity ? (
                   <tr><td colSpan={5} className="loading-state-row">Loading activity...</td></tr>
                  ...
                  recentActivity.map(activity => (
                    <tr key={activity.id} onClick={() => handleRowClick(activity)} className="clickable-row">
                      <td>{activity.requestType}</td>
                      <td>{activity.requestorName}</td>
                      <td>
                        <span className={`badge ${activity.action === 'approved' ? 'badge-success' : 'badge-error'}`}>
                          {activity.action}
                        </span>
                      </td>
        ...
        <div className="modal-overlay">
          <div className="modal-container">
            ...
            <div className="modal-content">
              <p><strong>Request Type:</strong> {selectedActivity.requestType}</p>
              <p><strong>Requestor:</strong> {selectedActivity.requestorName}</p>
              <p><strong>Action:</strong>
                <span className={`badge ${selectedActivity.action === 'approved' ? 'badge-success' : 'badge-error'}`}>
                  {selectedActivity.action}
                </span>
              </p>
              <p><strong>Actioned By:</strong> {selectedActivity.actionedByName} ({selectedActivity.actionedByUid})</p>
              <p><strong>Date:</strong> {formatTimestamp(selectedActivity.actionedTimestamp)}</p>
              {selectedActivity.details && <p><strong>Details:</strong> {selectedActivity.details}</p>}
            </div>
            <div className="modal-footer">
              <button type="button" className="dashboard-button" onClick={closeModal}>Close</button>
            </div>
          </div>
        </div>
      )}
