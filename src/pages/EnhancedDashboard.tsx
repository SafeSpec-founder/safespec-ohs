...
const EnhancedDashboard: React.FC = () => {
  ...
  const [upcomingMeetings] = useState<Meeting[]>([
    { id: '1', title: 'Safety Committee Meeting', date: 'Today, 2:00 PM', location: 'Conference Room A', attendees: 8 },
    { id: '2', title: 'Emergency Response Drill', date: 'Tomorrow, 10:00 AM', location: 'Main Facility', attendees: 25 },
    { id: '3', title: 'Hazard Assessment Review', date: 'May 22, 2025, 1:30 PM', location: 'Conference Room B', attendees: 6 }
  ]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (incidentExportRef.current && !incidentExportRef.current.contains(event.target as Node) && !incidentExportButtonRef.current?.contains(event.target as Node)) {
        setIncidentExportOpen(false);
      }
      if (complianceExportRef.current && !complianceExportRef.current.contains(event.target as Node) && !complianceExportButtonRef.current?.contains(event.target as Node)) {
  ...
  const handleDropdownKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>, type: 'incidents' | 'compliance') => {
    const menuRef = type === 'incidents' ? incidentExportRef : complianceExportRef;
    const buttonRef = type === 'incidents' ? incidentExportButtonRef : complianceExportButtonRef;
    const setOpen = type === 'incidents' ? setIncidentExportOpen : setComplianceExportOpen;

    if (!menuRef.current) return;

    const items = Array.from(menuRef.current.querySelectorAll<HTMLElement>('.dropdown-menu button'));
  ...
  useEffect(() => {
    if (incidentExportOpen && incidentExportRef.current) {
      incidentExportRef.current.querySelector<HTMLElement>('.dropdown-menu button')?.focus();
    }
  }, [incidentExportOpen]);

  useEffect(() => {
    if (complianceExportOpen && complianceExportRef.current) {
      complianceExportRef.current.querySelector<HTMLElement>('.dropdown-menu button')?.focus();
    }
    ...
    <div className="enhanced-dashboard">
      <div className="dashboard-header">
        ...
        <div className="dashboard-actions">
          <label htmlFor="timeRangeSelect" className="sr-only">Select Time Range</label> {/* Accessibility */}
          <select
            id="timeRangeSelect"
      ...
      <section aria-labelledby="metrics-heading" className="dashboard-metrics">
        <h2 id="metrics-heading" className="sr-only">Key Safety Metrics</h2>
        {/* Incident Rate Card */}
        <div className="metric-card" role="region" aria-labelledby="incident-rate-heading">
          ...
          <div className="metric-content">
            ...
            <div className="metric-value">
              ...
              <span className="trend positive" aria-label="Decrease of 0.3">-0.3</span>
            </div>
          </div>
        </div>
        {/* Compliance Rate Card */}
        <div className="metric-card" role="region" aria-labelledby="compliance-rate-heading">
          <div className="metric-icon" aria-hidden="true">
            {/* SVG */}
          </div>
          <div className="metric-content">
            ...
            <div className="metric-value">
              ...
              <span className="trend positive" aria-label="Increase of 2 percent">+2%</span>
            </div>
          </div>
        </div>
        {/* Near Misses Card */}
        <div className="metric-card" role="region" aria-labelledby="near-misses-heading">
          <div className="metric-icon" aria-hidden="true">
            {/* SVG */}
          </div>
          <div className="metric-content">
            ...
            <div className="metric-value">
              ...
              <span className="trend negative" aria-label="Increase of 2">+2</span>
            </div>
          </div>
        </div>
        {/* Safety Observations Card */}
        <div className="metric-card" role="region" aria-labelledby="safety-observations-heading">
          <div className="metric-icon" aria-hidden="true">
            {/* SVG */}
          </div>
          <div className="metric-content">
            ...
            <div className="metric-value">
              ...
              <span className="trend positive" aria-label="Increase of 12">+12</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="dashboard-row">
        ...
        <section aria-labelledby="incident-trends-heading" className={`dashboard-card ${expandedChart === 'incidents' ? 'expanded' : ''}`}>
          <div className="card-header">
            <h3 id="incident-trends-heading" className="card-title">Incident Trends</h3>
            <div className="card-actions">
              ...
              <div className="dropdown" ref={incidentExportRef}>
                ...
                {incidentExportOpen && (
                  ...
                  >
                    <button role="menuitem" onClick={() => exportChart('incidents', 'png')}>PNG Image</button>
                    <button role="menuitem" onClick={() => exportChart('incidents', 'pdf')}>PDF Document</button>
                    <button role="menuitem" onClick={() => exportChart('incidents', 'csv')}>CSV Data</button>
                  </div>
                )}
              </div>
              ...
              <button
                ...
                aria-label={expandedChart === 'incidents' ? 'Minimize Incident Trends Chart' : 'Expand Incident Trends Chart'}
              >
                {/* SVG */} 
              </button>
            </div>
          ...
          <div id="incident-chart-content" className="card-content">
            <div className="chart-container" aria-label="Incident Trends Bar Chart">
              ...
              <div className="placeholder-chart" role="img" aria-label="Bar chart showing incident trends over the last 5 months.">
                {/* ... placeholder bars ... */} 
              </div>
        ...
        <section aria-labelledby="compliance-metrics-heading" className={`dashboard-card ${expandedChart === 'compliance' ? 'expanded' : ''}`}>
          <div className="card-header">
            <h3 id="compliance-metrics-heading" className="card-title">Compliance Metrics</h3>
            <div className="card-actions">
              ...
              <div className="dropdown" ref={complianceExportRef}>
                ...
                {complianceExportOpen && (
                  ...
                  >
                    <button role="menuitem" onClick={() => exportChart('compliance', 'png')}>PNG Image</button>
                    <button role="menuitem" onClick={() => exportChart('compliance', 'pdf')}>PDF Document</button>
                    <button role="menuitem" onClick={() => exportChart('compliance', 'csv')}>CSV Data</button>
                  </div>
                )}
              </div>
              {/* Expand/Collapse Button */}
              <button
                ...
                aria-label={expandedChart === 'compliance' ? 'Minimize Compliance Metrics Chart' : 'Expand Compliance Metrics Chart'}
              >
                {/* SVG */} 
              </button>
            </div>
          ...
          <div id="compliance-chart-content" className="card-content">
            <div className="chart-container" aria-label="Compliance Metrics Donut Chart">
              {/* Placeholder Chart - Replace with actual charting library component */}
              <div className="placeholder-chart" role="img" aria-label="Donut chart showing 96% compliance.">
                {/* ... placeholder donut ... */} 
              </div>
              {/* Actual Chart Component would go here */}
      ...
      <section aria-labelledby="upcoming-meetings-heading" className="dashboard-card">
        <div className="card-header">
          <h3 id="upcoming-meetings-heading" className="card-title">Upcoming Meetings & Drills</h3>
            ...
            {upcomingMeetings.map(meeting => (
              <li key={meeting.id} className="meeting-item">
                ...
                <div className="meeting-details">
                  ...
                  <span className="meeting-meta">{meeting.date} | {meeting.location} | {meeting.attendees} Attendees</span>
                </div>
                <button className="meeting-action" aria-label={`View details for ${meeting.title}`}>
                  {/* SVG */} 
                </button>
              </li>
            ))}
