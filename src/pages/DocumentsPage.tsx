...
const DocumentsPage: React.FC = () => {
  ...
  return (
    <Container maxWidth="xl">
      ...
      {showUploader && (
        <Box sx={{ mb: 4 }}>
          <DocumentUploader onDocumentCreated={handleUploadSuccess} onCancel={() => setShowUploader(false)} />
        </Box>
      )}
      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab label={`Policies (${documents.filter(doc => doc.category === 'policy').length})`} {...a11yProps(1)} />
            <Tab label={`Procedures (${documents.filter(doc => doc.category === 'procedure').length})`} {...a11yProps(2)} />
            <Tab label={`Forms (${documents.filter(doc => doc.category === 'form').length})`} {...a11yProps(3)} />
            <Tab label={`Reports (${documents.filter(doc => doc.category === 'report').length})`} {...a11yProps(4)} />
          </Tabs>
        </Box>
...
const DocumentsList: React.FC<DocumentsListProps> = ({ documents, onView, onDownload, onEdit, onDelete }) => {
