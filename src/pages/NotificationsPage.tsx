...
const NotificationsPage: React.FC = () => {
  ...
  const filteredNotifications = React.useMemo(() => {
    switch (value) {
      ...
      case 2: // Incidents
        return notifications.filter(notification => notification.type === 'incident');
      case 3: // Tasks
        return notifications.filter(notification => notification.type === 'task');
      case 4: // System
      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab label={`Unread (${notifications.filter(notification => !notification.read).length})`} {...a11yProps(1)} />
            <Tab label={`Incidents (${notifications.filter(notification => notification.type === 'incident').length})`} {...a11yProps(2)} />
            <Tab label={`Tasks (${notifications.filter(notification => notification.type === 'task').length})`} {...a11yProps(3)} />
            <Tab label={`System (${notifications.filter(notification => notification.type === 'system').length})`} {...a11yProps(4)} />
          </Tabs>
        </Box>
