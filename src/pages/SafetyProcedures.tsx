...
const SafetyProcedures: React.FC = () => {
  ...
  const [procedureCategories] = useState<ProcedureCategory[]>([
    { id: 'cat-1', name: 'Emergency Response', icon: '🚨', description: 'Handling emergencies', count: 0 }, // Counts will update dynamically
    { id: 'cat-2', name: 'Hazardous Materials', icon: '☣️', description: 'Chemical safety', count: 0 },
    { id: 'cat-3', name: 'Equipment Safety', icon: '⚙️', description: 'Machine operation', count: 0 },
    { id: 'cat-4', name: 'Personal Safety', icon: '👷', description: 'PPE and practices', count: 0 },
    { id: 'cat-5', name: 'Workplace Safety', icon: '🏢', description: 'General site safety', count: 0 },
    { id: 'cat-6', name: 'Reporting', icon: '📊', description: 'Incident reporting', count: 0 },
  ]);

  const [procedureUpdates] = useState<ProcedureUpdate[]>([
    // Replace with dynamic updates from Firestore if needed
    { id: 'upd-1', title: 'Emergency Evacuation Procedure updated', user: 'System', time: 'May 25, 2025' },
    { id: 'upd-2', title: 'Chemical Handling Guidelines updated', user: 'System', time: 'May 24, 2025' },
  ]);
  // --- End Static Data ---
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (typeof data.lastUpdated === 'object' && data.lastUpdated !== null) {
      ...
      addToast({ type: 'error', title: 'Error', message: 'Could not load safety procedures.' });
      setIsLoading(false);
    });
  ...
  const filteredProcedures = procedures.filter(procedure => {
    ...
    const matchesCategory =
      filterCategory === 'all' ||
      procedure.category === filterCategory;
  ...
  const handleViewProcedure = (procedure: Procedure) => {
    ...
    if (!procedure.content) {
        addToast({ type: 'info', title: 'Loading Content', message: 'Fetching procedure details...' });
        // Example: Fetch content if not already loaded
        const procedureDocRef = doc(db, 'safetyProcedures', procedure.id);
        getDoc(procedureDocRef).then(docSnap => {
            if (docSnap.exists()) {
                ...
                addToast({ type: 'error', title: 'Error', message: 'Could not load procedure content.' });
            }
        }).catch(error => {
  ...
  const handleDownloadProcedure = (procedure: Procedure) => {
    ...
    try {
        ...
        const splitContent = doc.splitTextToSize(procedure.content || 'No content available.', 180);
  ...
  const handleCategoryFilter = (category: string) => {
    ...
    addToast({
      ...
      message: category === 'all' ? 'Showing all procedures' : `Filtering by category: ${category}`,
      duration: 2000
    });
  };
  ...
  const handleAddCategory = () => {
    addToast({ type: 'info', title: 'Add Category', message: 'Functionality not implemented yet.' });
  };
  const handleEditCategory = (categoryId: string) => {
    addToast({ type: 'info', title: 'Edit Category', message: `Edit category ${categoryId} - Functionality not implemented yet.` });
  };
  const handleDeleteCategory = (categoryId: string) => {
    addToast({ type: 'info', title: 'Delete Category', message: `Delete category ${categoryId} - Functionality not implemented yet.` });
  };
    ...
    <div className="dashboard-container safety-procedures-page">
      ...
      <div className="filter-container">
        <div className="search-box">
          {/* SVG Icon */}
          <label htmlFor="procedureSearch" className="sr-only">Search Procedures</label>
          <input
            id="procedureSearch"
            type="search"
        ...
        <div className="filter-box">
          ...
          <select id="categoryFilter" value={filterCategory} onChange={(e) => handleCategoryFilter(e.target.value)}>
            <option value="all">All Categories</option>
            {procedureCategories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
          </select>
        </div>
      </div>
              ...
              <>
                {filteredProcedures.length === 0 ? (
                  <div className="empty-state" style={{ padding: '2rem' }}>
                    ...
                    <p>No procedures match your current search term "{searchTerm}"{filterCategory !== 'all' ? ` and category "${filterCategory}"` : ''}.</p>
                    <button
                  ...
                  <div ref={tableContainerRef} className="table-container">
                    <div className="table-scroll-container" style={{ maxHeight: 'calc(10 * 55px)', overflowY: 'auto' }}> {/* Adjust max height */}
                      <table id="procedures-table" className="data-table">
                        <caption className="sr-only">List of Safety Procedures</caption>
                        <thead>
                          <tr>
                            <th scope="col">ID</th>
                        ...
                        <tbody>
                          {filteredProcedures.map(procedure => (
                            <tr key={procedure.id}>
                              ...
                              <td>
                                <span className={`badge ${getStatusBadgeClass(procedure.status)}`}>
                                  {procedure.status}
                                </span>
                              </td>
                              <td>
        ...
        <div className="modal-overlay">
          <div className="modal-container large">
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={closeProcedureModal} aria-label="Close modal">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="modal-footer">
              <button className="dashboard-button" onClick={closeProcedureModal}>
                Close
              </button>
              <button
                className="dashboard-button primary"
