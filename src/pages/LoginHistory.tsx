...
const LoginHistory: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      addToast({ type: 'error', title: 'Error', message: 'Could not load system logs.' });
      setLoading(false);
    });
  ...
  if (userRole !== 'admin') {
    return <div className="access-denied">Access Denied. You must be an administrator to view system logs.</div>;
  }
    ...
    <div className="dashboard-container system-logs">
      ...
      <div className="dashboard-card">
        <div className="card-header">
          <div className="card-title">Recent Activity ({loading ? '...' : filteredLogs.length})</div>
        ...
        <div className="card-content no-padding">
          <div className="logs-table-container">
            <table className="data-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={5} className="loading-state-row">Loading logs...</td></tr>
                ) : filteredLogs.length === 0 ? (
                  <tr><td colSpan={5} className="empty-state-row">No matching logs found.</td></tr>
                ) : (
