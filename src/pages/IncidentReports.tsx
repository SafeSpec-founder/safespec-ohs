...
const IncidentReports: React.FC = () => {
  ...
  const handleDownload = (incidentId: string): void => {
    ...
    if (incident) {
      addToast({ type: 'info', title: 'Generating Report', message: `Preparing PDF for incident ${incident.title}...` });
      // Simulate PDF generation
      setTimeout(() => {
        const blob = new Blob([`Incident Report\n------------------\nID: ${incident.id}\nTitle: ${incident.title}\nLocation: ${incident.location}\nDate: ${incident.date}\nSeverity: ${incident.severity}\nStatus: ${incident.status}\n\nDetails: [Placeholder for full report details]` ], { type: 'application/pdf' });
        ...
        addToast({ type: 'success', title: 'Report Downloaded', message: `PDF for incident ${incident.title} downloaded.` });
      }, 1000);
    }
  };
  ...
  const handleReportIncident = () => {
    addToast({ type: 'info', title: 'Report Incident', message: 'Navigating to incident reporting form (not implemented).' });
    ...
    <div className="dashboard-container incident-reports">
      <div className="dashboard-header">
        ...
        <div className="dashboard-actions">
          <button className="dashboard-button primary" onClick={handleReportIncident}>
                      ...
                      <td>
                        <div className="action-buttons">
                          <button
                            ...
                            title="View Details"
                            onClick={() => addToast({ type: 'info', title: 'View Incident', message: `Viewing details for incident ${incident.id} (not implemented).` })}
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          {(incident.status === 'Open' || incident.status === 'In Progress') && (
                            <button
                              className="action-button"
                              title="Edit Incident"
                              onClick={() => addToast({ type: 'info', title: 'Edit Incident', message: `Editing incident ${incident.id} (not implemented).` })}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          {userRole === 'admin' && incident.status !== 'Closed' && incident.status !== 'Resolved' && (
                             <button
                              className="action-button"
                              title="Close Incident"
                              onClick={() => addToast({ type: 'info', title: 'Close Incident', message: `Closing incident ${incident.id} (not implemented).` })}
                            >
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
