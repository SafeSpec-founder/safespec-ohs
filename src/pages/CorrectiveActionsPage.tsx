...
import { selectCorrectiveActions, fetchCorrectiveActions } from '@store/slices/correctiveActionSlice';
...
const CorrectiveActionsPage: React.FC = () => {
  ...
  const filteredActions = React.useMemo(() => {
    switch (value) {
      ...
      case 2: // In Progress
        return correctiveActions.filter(action => action.status === 'in_progress');
      case 3: // Completed
        return correctiveActions.filter(action => action.status === 'completed');
      case 4: // Overdue
        return correctiveActions.filter(action => {
          return new Date(action.dueDate) < new Date() && action.status !== 'completed';
        });
      default:
        return correctiveActions;
    }
  ...
  return (
    <Container maxWidth="xl">
      ...
      {showForm && (
        <Box sx={{ mb: 4 }}>
          <CorrectiveActionForm onSuccess={handleCreateSuccess} onCancel={() => setShowForm(false)} />
        </Box>
      )}
      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs
            ...
            scrollButtons="auto"
          >
            <Tab label={`All (${correctiveActions.length})`} {...a11yProps(0)} />
            <Tab label={`Open (${correctiveActions.filter(action => action.status === 'open').length})`} {...a11yProps(1)} />
            <Tab label={`In Progress (${correctiveActions.filter(action => action.status === 'in_progress').length})`} {...a11yProps(2)} />
            <Tab label={`Completed (${correctiveActions.filter(action => action.status === 'completed').length})`} {...a11yProps(3)} />
            <Tab 
              label={`Overdue (${correctiveActions.filter(action => {
                return new Date(action.dueDate) < new Date() && action.status !== 'completed';
              }).length})`} 
              {...a11yProps(4)} 
            />
...
const ActionsList: React.FC<ActionsListProps> = ({ actions, onEdit, onDelete, onComplete, onVerify }) => {
  if (actions.length === 0) {
    return (
