...
const IncidentsPage: React.FC = () => {
  ...
  const filteredIncidents = React.useMemo(() => {
    switch (value) {
      ...
      case 2: // Critical
        return incidents.filter(inc => inc.severity === 'critical' || inc.severity === 'high');
      case 3: // Closed
        return incidents.filter(inc => inc.status === 'closed');
      default:
        return incidents;
  ...
  return (
    <Container maxWidth="xl">
      ...
      {showForm && (
        <Box sx={{ mb: 4 }}>
          <IncidentForm onSuccess={handleCreateSuccess} onCancel={() => setShowForm(false)} />
        </Box>
      )}
      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab label={`Open (${incidents.filter(inc => inc.status !== 'closed').length})`} {...a11yProps(1)} />
            <Tab label={`Critical (${incidents.filter(inc => inc.severity === 'critical' || inc.severity === 'high').length})`} {...a11yProps(2)} />
            <Tab label={`Closed (${incidents.filter(inc => inc.status === 'closed').length})`} {...a11yProps(3)} />
          </Tabs>
        </Box>
...
const IncidentsList: React.FC<IncidentsListProps> = ({ incidents, onView, onEdit, onDelete }) => {
