  ...
  const filteredReports = React.useMemo(() => {
    switch (value) {
      ...
      case 1: // Incident Reports
        return reports.filter(report => report.type === 'incident');
      case 2: // Compliance Reports
        return reports.filter(report => report.type === 'compliance');
      case 3: // Safety Metrics
        return reports.filter(report => report.type === 'safety_metrics');
      case 4: // Custom Reports
        return reports.filter(report => report.type === 'custom');
      default:
        return reports;
    }
  }, [reports, value]);
    ...
    <Container maxWidth="xl">
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          ...
          >
            <TableChartIcon sx={{ fontSize: 48, color: 'warning.main', mb: 1 }} />
            <Typography variant="h6" gutterBottom>
              Custom Report
            </Typography>
            <Typography variant="body2" color="text.secondary">
      ...
      <Paper sx={{ width: '100%', borderRadius: 2 }}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <Tabs
            ...
            scrollButtons="auto"
          >
            <Tab label={`All Reports (${reports.length})`} {...a11yProps(0)} />
            <Tab label={`Incident Reports (${reports.filter(report => report.type === 'incident').length})`} {...a11yProps(1)} />
            <Tab label={`Compliance Reports (${reports.filter(report => report.type === 'compliance').length})`} {...a11yProps(2)} />
            <Tab label={`Safety Metrics (${reports.filter(report => report.type === 'safety_metrics').length})`} {...a11yProps(3)} />
            <Tab label={`Custom Reports (${reports.filter(report => report.type === 'custom').length})`} {...a11yProps(4)} />
          </Tabs>
        </Box>
    ...
    <Grid container spacing={3} sx={{ p: 2 }}>
      {reports.map((report) => (
        <Grid item xs={12} sm={6} md={4} lg={3} key={report.id}>
          <ReportCard report={report} onView={() => onView(report)} />
        </Grid>
      ))}
    </Grid>
  );
