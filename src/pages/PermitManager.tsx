...
interface CreatePermitModalProps {
  onClose?: () => void;
  onCreate: (newPermitData: Omit<Permit, 'id' | 'issuedBy' | 'status'> & { status?: Permit['status'] }) => void; // Adjust based on creation logic
}

// --- Create Permit Modal Component ---
const CreatePermitModal: React.FC<CreatePermitModalProps> = ({ onClose, onCreate }) => {
  const [permitName, setPermitName] = useState('');
  const [permitType, setPermitType] = useState('Hot Work'); // Default type
  const [location, setLocation] = useState('');
  const [issuedTo, setIssuedTo] = useState('');
  const [issuedDate, setIssuedDate] = useState(new Date().toISOString().split('T')[0]); // Default to today
  const [expiryDate, setExpiryDate] = useState('');
  const [document, setDocument] = useState<File | null>(null);
  const { addToast } = useToast();

  const permitTypes = ['Hot Work', 'Confined Space', 'Excavation', 'Electrical', 'Heights', 'Chemical'];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!permitName || !location || !issuedTo || !issuedDate || !expiryDate) {
      addToast({ type: 'error', title: 'Missing Information', message: 'Please fill in all required fields.' });
      return;
    }
    // Basic date validation
    if (new Date(expiryDate) <= new Date(issuedDate)) {
        addToast({ type: 'error', title: 'Invalid Dates', message: 'Expiry date must be after issued date.' });
        return;
    }
    ...
    console.log('Creating permit with data:', newPermitData, 'Document:', document?.name);
    addToast({ type: 'info', title: 'Creating Permit', message: 'Submitting permit request...' });
    // Simulate creation
    setTimeout(() => {
        onCreate(newPermitData);
        addToast({ type: 'success', title: 'Permit Created', message: `Permit '${permitName}' submitted for approval.` });
        if (onClose) onClose();
    }, 1000);
  };
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-container large"> {/* Added 'large' class for wider modal */}
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-content">
            <div className="form-grid two-column"> {/* Use two-column grid */}
              {/* Column 1 */}
              ...
              <div className="form-column">
                ...
                <div className="form-group">
                  <label htmlFor="documentUpload">Attach Supporting Document (Optional)</label>
                  <input
                    type="file"
                    id="documentUpload"
          ...
          <div className="modal-footer">
            <button type="button" className="dashboard-button" onClick={onClose}>
              Cancel
            </button>
...
const PermitManager: React.FC = () => {
  ...
  const [permitStats, setPermitStats] = useState<PermitStats>({ active: 0, pending: 0, expired: 0, closed: 0, revoked: 0 });
  ...
  const calculateStats = (currentPermits: Permit[]): PermitStats => {
    return currentPermits.reduce((acc, permit) => {
  ...
  const permitTypes = ['All', 'Hot Work', 'Confined Space', 'Excavation', 'Electrical', 'Heights', 'Chemical'];
  const statusOptions = ['All', 'Active', 'Pending Approval', 'Expired', 'Closed', 'Revoked'];
  ...
  const handleCreatePermit = async (newPermitData: Omit<Permit, 'id' | 'issuedBy' | 'status'> & { status?: Permit['status'] }): Promise<void> => {
    try {
  ...
  const handleStatusChange = async (id: string, newStatus: Permit['status']): Promise<void> => {
    try {
  ...
  const handlePrintPermit = async (permit: Permit) => {
    try {
      addToast({ type: 'info', title: 'Printing Permit', message: `Generating print view for ${permit.name}...` });
      ...
      addToast({ type: 'success', title: 'Print Ready', message: `Print dialog opened for ${permit.name}.` });
    } catch (error) {
  ...
  const getStatusBadgeClass = (status: string): string => {
    switch(status) {
      ...
      case 'Revoked': return 'badge-error'; // Use error style for revoked
  ...
  const getStatsIconClass = (type: keyof PermitStats): string => {
    switch(type) {
      ...
      case 'revoked': return 'revoked'; // Added revoked style
      default: return '';
    }
  };
    ...
    <div className="dashboard-container permit-manager">
      ...
      <div className="dashboard-card">
        ...
        <div className="card-content no-padding">
          {isLoading ? (
            <div className="loader-container"><div className="loader"></div><div className="loader-text">Loading permits...</div></div>
          ) : (
            <div className="table-container">
              <div className="table-scroll-container" style={{ maxHeight: 'calc(6 * 55px)', overflowY: 'auto' }}>
                <table className="data-table">
                  ...
                  <tbody>
                    {filteredPermits.length === 0 ? (
                      <tr><td colSpan={8} className="empty-state"><div><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path><polyline points="13 2 13 9 20 9"></polyline></svg><h3>No permits found</h3><p>Try adjusting your search or filters</p></div></td></tr>
                    ) : (
                      filteredPermits.map(permit => (
                        <tr key={permit.id}>
                          ...
                          <td><span className={`badge ${getStatusBadgeClass(permit.status)}`}>{permit.status}</span></td>
                          <td>{permit.issuedTo}</td>
                          <td>{new Date(permit.issuedDate).toLocaleDateString()}</td>
                          <td>{new Date(permit.expiryDate).toLocaleDateString()}</td>
                          <td>
                            <div className="action-buttons">
                              {/* View Button */}
                              <button className="action-button tooltip" onClick={() => { setSelectedPermit(permit); setShowViewModal(true); }} aria-label="View permit details">
                                <span className="tooltip-content">View</span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                              </button>
                              {/* Print Button */}
                              <button className="action-button tooltip" onClick={() => handlePrintPermit(permit)} aria-label="Print permit">
                              ...
                              {userRole === 'admin' && (
                                <>
                                  {/* Edit Button */}
                                  <button className="action-button tooltip" onClick={() => handleEditPermit(permit)} aria-label="Edit permit">
                                    <span className="tooltip-content">Edit</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                  </button>
                                  {/* Revoke Button */}
                                  {permit.status === 'Active' && (
                                    <button className="action-button tooltip danger" onClick={() => handleStatusChange(permit.id, 'Revoked')} aria-label="Revoke permit">
                                      <span className="tooltip-content">Revoke</span>
                                  ...
                                  {permit.status === 'Pending Approval' && (
                                    <button className="action-button tooltip success" onClick={() => handleStatusChange(permit.id, 'Active')} aria-label="Approve permit">
                                  ...
                                  {permit.status === 'Pending Approval' && (
                                    <button className="action-button tooltip danger" onClick={() => handleStatusChange(permit.id, 'Revoked')} aria-label="Decline permit">
                                      <span className="tooltip-content">Decline</span>
          ...
          {isLoading ? (
            ...
            <div className="stats-grid">
              ...
              <div className="stat-card">
                <div className={`stat-icon ${getStatsIconClass('active')}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></div>
                <div className="stat-content"><div className="stat-title">Active Permits</div><div className="stat-value">{permitStats.active}</div></div>
              </div>
              {/* Pending */}
              <div className="stat-card">
                <div className={`stat-icon ${getStatsIconClass('pending')}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg></div>
                <div className="stat-content"><div className="stat-title">Pending Approval</div><div className="stat-value">{permitStats.pending}</div></div>
              </div>
              {/* Expired */}
              <div className="stat-card">
                <div className={`stat-icon ${getStatsIconClass('expired')}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg></div>
                <div className="stat-content"><div className="stat-title">Expired Permits</div><div className="stat-value">{permitStats.expired}</div></div>
              </div>
              {/* Closed */}
              <div className="stat-card">
                <div className={`stat-icon ${getStatsIconClass('closed')}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18"></path><path d="M6 6l12 12"></path></svg></div>
                <div className="stat-content"><div className="stat-title">Closed Permits</div><div className="stat-value">{permitStats.closed}</div></div>
              </div>
               {/* Revoked */}
              <div className="stat-card">
                <div className={`stat-icon ${getStatsIconClass('revoked')}`}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="8" y1="12" x2="16" y2="12"></line></svg></div>
                <div className="stat-content"><div className="stat-title">Revoked Permits</div><div className="stat-value">{permitStats.revoked}</div></div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="modal-overlay">
          <div className="modal-container">
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowViewModal(false)} aria-label="Close modal">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
              </button>
            </div>
            <div className="modal-content">
              <div className="report-details">
                <div className="detail-row"><div className="detail-label">Permit ID:</div><div className="detail-value">{selectedPermit.id}</div></div>
                <div className="detail-row"><div className="detail-label">Type:</div><div className="detail-value">{selectedPermit.type}</div></div>
                <div className="detail-row"><div className="detail-label">Location:</div><div className="detail-value">{selectedPermit.location}</div></div>
                <div className="detail-row"><div className="detail-label">Status:</div><div className="detail-value"><span className={`badge ${getStatusBadgeClass(selectedPermit.status)}`}>{selectedPermit.status}</span></div></div>
                <div className="detail-row"><div className="detail-label">Issued To:</div><div className="detail-value">{selectedPermit.issuedTo}</div></div>
                <div className="detail-row"><div className="detail-label">Issued By:</div><div className="detail-value">{selectedPermit.issuedBy}</div></div>
                <div className="detail-row"><div className="detail-label">Issued Date:</div><div className="detail-value">{new Date(selectedPermit.issuedDate).toLocaleDateString()}</div></div>
                <div className="detail-row"><div className="detail-label">Expiry Date:</div><div className="detail-value">{new Date(selectedPermit.expiryDate).toLocaleDateString()}</div></div>
              </div>
              <div className="report-content">
                <h3>Permit Details / Conditions</h3>
                <p>Placeholder for additional permit details, conditions, or associated documents.</p>
              </div>
            </div>
            <div className="modal-footer">
              <button className="dashboard-button" onClick={() => setShowViewModal(false)}>Close</button>
              <button className="dashboard-button primary" onClick={() => handlePrintPermit(selectedPermit)}>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
                Print Permit
              </button>
            </div>
