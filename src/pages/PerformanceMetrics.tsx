...
const getMetricStatus = (key: MetricKey, value: number): { text: string; className: string } => {
  ...
  if (isLowerBetter) {
    if (value <= metricThresholds.good) return { text: 'Good', className: 'badge-success' };
    if (value <= metricThresholds.warning) return { text: 'Needs Attention', className: 'badge-warning' };
...
const formatTrend = (key: MetricKey, trend: number | undefined): { text: string; className: string; ariaLabel: string } => {
  if (trend === undefined) return { text: 'N/A', className: 'trend neutral', ariaLabel: 'Trend: Not available' };
  const isLowerBetter = key === 'incidentRate' || key === 'nearMisses';
  const isPositive = isLowerBetter ? trend < 0 : trend > 0;
  const isNegative = isLowerBetter ? trend > 0 : trend < 0;
  const unit = key !== 'incidentRate' && key !== 'nearMisses' && key !== 'safetyObservations' ? '%' : '';
  ...
  return { text: trendText, className, ariaLabel: `Trend: ${trendDescription}` };
};

const formatMetricValue = (key: MetricKey, value: number): string => {
  if (key === 'incidentRate' || key === 'nearMisses' || key === 'safetyObservations') {
...
const PerformanceMetrics: React.FC = () => {
  ...
  const [detailedChartData, setDetailedChartData] = useState<DetailedDataPoint[]>([]);
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      ...
      try {
        ...
        const prevMetricsQuery = query(historyCollectionRef, orderBy('timestamp', 'desc'), limit(2));
        ...
        if (prevMetricsSnap.docs.length > 1) {
          ...
          previousData = prevMetricsSnap.docs[1].data() as Record<string, number>;
        }
        ...
        addToast({ type: 'error', title: 'Error', message: 'Could not load performance metrics.' });
  ...
  useEffect(() => {
    ...
    const fetchDetailedData = async () => {
      try {
        ...
        querySnapshot.docs.reverse().forEach((doc) => { // Reverse to show oldest first
          const data = doc.data() as MetricHistoryPoint;
          fetchedData.push({
            // Format timestamp to month/year or desired label
            month: data.timestamp.toDate().toLocaleString('default', { month: 'short', year: 'numeric' }),
            value: data.value,
          });
        });
        setDetailedChartData(fetchedData);
      } catch (error) {
        console.error(`Error fetching detailed data for ${selectedMetric}: `, error);
        addToast({ type: 'error', title: 'Error', message: `Could not load detailed data for ${getMetricName(selectedMetric)}.` });
  ...
  useEffect(() => {
    ...
    if (showDetailedView && selectedMetric && chartRef.current && detailedChartData.length > 0) {
  ...
  const exportMetricsData = (format: 'pdf' | 'excel'): void => {
    ...
    addToast({ type: 'info', title: 'Exporting Data', message: `Preparing ${format.toUpperCase()} export...` });
    console.log(`Exporting metrics data in ${format} format`);
    // TODO: Implement actual export logic (e.g., using jsPDF, SheetJS)
    setTimeout(() => {
      setIsExporting(false);
      setExportFormat(null);
      addToast({ type: 'success', title: 'Export Ready', message: `Metrics data export (${format.toUpperCase()}) is ready.` });
  ...
  // --- Render Helpers ---
  const renderMetricCard = (metricKey: MetricKey, icon: React.ReactNode): React.ReactNode => {
    ...
    return (
      <div className="metric-card" onClick={() => showMetricDetails(metricKey)} tabIndex={0} role="button" aria-label={`View details for ${getMetricName(metricKey)}`}>
        <div className="metric-header">
          <div className="metric-icon">{icon}</div>
          <div className="metric-title">{getMetricName(metricKey)}</div>
        </div>
        <div className="metric-value">{formatMetricValue(metricKey, metricData.value)}</div>
        <div className="metric-footer">
          <div className={`metric-status ${status.className}`}>{status.text}</div>
          <div className={trend.className} aria-label={trend.ariaLabel}>{trend.text}</div>
        </div>
        <div className="metric-source">Source: {getMetricSource(metricKey)}</div>
      </div>
    );
  };
  ...
  return (
    <div className="performance-metrics-container">
          ...
          <div className="export-container">
            ...
            {showExportOptions && (
              <div ref={exportMenuRef} className="export-options">
                ...
                <button onClick={() => exportMetricsData('excel')}>Excel</button>
              </div>
            )}
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="error-container">Could not load metrics data. Please try again later.</div>
      ) : (
        <div className="metrics-grid">
          ...
          {metrics.hseTargetProgress && renderMetricCard('hseTargetProgress', <span>🎯</span>)}
          {metrics.nearMisses && renderMetricCard('nearMisses', <span>⚠️</span>)}
          {metrics.safetyObservations && renderMetricCard('safetyObservations', <span>👁️</span>)}
      ...
      {showDetailedView && selectedMetric && (
        <div className="modal-overlay" onClick={closeDetailedView}> {/* Close on overlay click */}
          <div ref={modalRef} className="modal-container detailed-metric-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
            <div className="modal-header">
              <h2 id="modal-title">Detailed View: {getMetricName(selectedMetric)}</h2>
              <button ref={modalCloseButtonRef} type="button" className="close-button" onClick={closeDetailedView} aria-label="Close detailed view">
                &times;
              </button>
            </div>
            <div className="modal-body">
              <div className="metric-details">
                <div className="metric-info">
                  <p><strong>Source:</strong> {getMetricSource(selectedMetric)}</p>
                  <p><strong>Current Value:</strong> {metrics && metrics[selectedMetric] ? formatMetricValue(selectedMetric, metrics[selectedMetric].value) : 'N/A'}</p>
                  <p><strong>Status:</strong> {metrics && metrics[selectedMetric] ? getMetricStatus(selectedMetric, metrics[selectedMetric].value).text : 'N/A'}</p>
                  <p><strong>Trend:</strong> {metrics && metrics[selectedMetric] ? formatTrend(selectedMetric, metrics[selectedMetric].trend).text : 'N/A'}</p>
                </div>
                <div className="chart-container">
                  ...
                  {detailedChartData.length > 0 ? (
                    ...
                    <div className="no-data-message">No historical data available for this metric.</div>
                  )}
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button type="button" className="secondary-button" onClick={closeDetailedView}>Close</button>
              <button type="button" className="primary-button" onClick={() => exportMetricsData('pdf')}>Export as PDF</button>
            </div>
          </div>
        </div>
      )}
