...
export const documentService = {
  async getDocuments(params: { page?: number; limit?: number; filters?: any } = {}): Promise<{ data: Document[]; total: number }> {
  ...
  async createDocument(document: Omit<Document, 'id' | 'version' | 'createdAt' | 'updatedAt'>): Promise<Document> {
  ...
  async uploadDocumentFile(id: string, file: File): Promise<{ id: string; fileUrl: string; fileType: string; fileSize: number }> {
