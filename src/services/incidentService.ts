...
export const incidentService = {
  async getIncidents(params: { page?: number; limit?: number; filters?: any } = {}): Promise<{ data: Incident[]; total: number }> {
  ...
  async uploadAttachment(incidentId: string, file: File): Promise<{ id: string; url: string; filename: string; contentType: string }> {
  ...
  async assignIncident(incidentId: string, userId: string): Promise<Incident> {
    const response = await api.post(`/incidents/${incidentId}/assign`, { userId });
    return response.data;
  },
  
  async closeIncident(incidentId: string, resolution: string): Promise<Incident> {
    const response = await api.post(`/incidents/${incidentId}/close`, { resolution });
    return response.data;
  },
  
  async reopenIncident(incidentId: string, reason: string): Promise<Incident> {
    const response = await api.post(`/incidents/${incidentId}/reopen`, { reason });
    return response.data;
  },
  
  async addComment(incidentId: string, comment: string): Promise<{ id: string; comment: string; createdBy: string; createdAt: string }> {
  ...
  async exportIncidents(format: 'csv' | 'pdf' | 'excel', filters?: any): Promise<Blob> {
    const queryParams = new URLSearchParams();
    ...
    const response = await api.get(`/incidents/export?${queryParams.toString()}`, {
      responseType: 'blob',
    });
