...
export const chatService = {
  ...
  async getMessages(conversationId: string): Promise<ChatMessage[]> {
    ...
    // Cache the response for offline access
    await cacheResponse(`/chat/conversations/${conversationId}/messages`, response.data);
    
    return response.data;
  },
  
  async sendMessage(conversationId: string | null, content: string, metadata?: any): Promise<any> {
    ...
    if (!conversationId) {
      ...
      const messageResponse = await this.sendMessage(conversationId, content, metadata);
  ...
  async analyzeIncident(incidentId: string): Promise<{ analysis: string; recommendations: string[] }> {
    const response = await api.post('/chat/analyze-incident', { incidentId });
    return response.data;
  },
  
  async generateCorrectiveActions(incidentId: string): Promise<{ actions: string[] }> {
    const response = await api.post('/chat/generate-corrective-actions', { incidentId });
    return response.data;
  },
  
  async askSafetyQuestion(question: string): Promise<{ answer: string; references: string[] }> {
