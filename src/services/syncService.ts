...
class SyncService {
  ...
  private saveSyncQueue() {
    try {
      localStorage.setItem('safespec-sync-queue', JSON.stringify(this.syncQueue));
    } catch (error) {
      console.error('Failed to save sync queue:', error);
    }
  }

  public addToSyncQueue(item: Omit<SyncItem, 'id' | 'timestamp' | 'retryCount'>) {
  ...
  private async syncItem(item: SyncItem): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        console.log(`Successfully synced ${item.type} ${item.action} for item ${item.id}`);
        return true;
      } else {
...
export const addToSyncQueue = (item: Omit<SyncItem, 'id' | 'timestamp' | 'retryCount'>) => {
