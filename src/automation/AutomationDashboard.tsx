...
import { Box, Typography, Paper, Grid, Button, Card, CardContent, CardActions, Chip, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem, CircularProgress } from '@mui/material';
...
const AutomationDashboard: React.FC = () => {
  ...
  const handleSaveRule = async (ruleData: Partial<Rule>) => {
    ...
    try {
      if (isOffline) {
        queueAction('saveRule', { ruleId: selectedRule?.id, ruleData });
        // Optimistically update UI
        if (selectedRule) {
          // Update existing rule
          const updatedRules = rules.map(rule => 
            rule.id === selectedRule.id ? { ...rule, ...ruleData, updatedAt: new Date() } : rule
          );
      ...
      if (selectedRule) {
        // Update existing rule
        savedRule = await automationService.updateRule(selectedRule.id, ruleData);
        const updatedRules = rules.map(rule => 
  ...
  const handleToggleRuleStatus = async (ruleId: string, enabled: boolean) => {
    try {
      if (isOffline) {
        queueAction('toggleRuleStatus', { ruleId, enabled });
        // Optimistically update UI
        const updatedRules = rules.map(rule => 
      ...
      setError(`Failed to ${enabled ? 'enable' : 'disable'} rule. Please try again.`);
  ...
  const handleDeleteRule = async () => {
    ...
    try {
      if (isOffline) {
        queueAction('deleteRule', { ruleId: ruleToDelete });
        // Optimistically update UI
  ...
  const handleViewRuleHistory = (ruleId: string) => {
    // Navigate to rule history page
    window.location.href = `/automation/rules/${ruleId}/history`;
  };
  ...
  const formatTriggerDescription = (rule: Rule): string => {
    ...
    if (type === 'event') {
      ...
      if (config.frequency === 'daily') {
        ...
        const days = config.days.map((day: string) => day.charAt(0).toUpperCase() + day.slice(1)).join(', ');
        return `Weekly on ${days} at ${config.time}`;
      } else if (config.frequency === 'monthly') {
        return `Monthly at ${config.time}`;
      } else {
        return `${config.frequency.charAt(0).toUpperCase() + config.frequency.slice(1)} at ${config.time}`;
      }
      ...
      switch (operator) {
        ...
        case 'is_empty': operatorText = 'is empty'; break;
        case 'is_not_empty': operatorText = 'is not empty'; break;
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 3, textAlign: 'center' }}>
                ...
                <Typography variant="body1" paragraph>
                  Create your first automation rule to streamline your safety workflows.
                </Typography>
                <Button 
                  variant="contained" 
                  color="primary"
                ...
                <Card 
                  sx={{ 
                    ...
                    borderColor: rule.enabled ? 
                      (rule.status === 'error' ? 'error.main' : 'success.main') : 
                      'grey.400'
                  }}
                >
