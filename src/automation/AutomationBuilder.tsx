...
const AutomationBuilder = () => {
  ...
  const { automationRules } = useSelector((state: RootState) => state.automation);
  ...
  const handleTriggerTypeChange = (triggerType: any) => {
    setRule(prev => ({
      ...
      triggerConfig: {
        ...
        condition: triggerType === 'condition' ? prev.triggerConfig.condition : {
          entity: '',
          field: '',
          operator: 'equals',
          value: ''
  ...
  const renderStepContent = (step: number) => {
    switch (step) {
              ...
              <Card key={index} sx={{ mb: 2 }}>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    <Typography variant="subtitle1">Action {index + 1}</Typography>
                    <IconButton
      ...
      case 3:
        return (
          <Box>
            <Typography variant="h6" gutterBottom>Review Your Rule</Typography>
            
            <Paper sx={{ p: 3, mb: 3 }}>
              <Typography variant="subtitle1" gutterBottom>Basic Information</Typography>
              <Typography><strong>Name:</strong> {rule.name}</Typography>
              <Typography><strong>Description:</strong> {rule.description}</Typography>
              <Typography><strong>Priority:</strong> {rule.priority}</Typography>
              <Typography><strong>Status:</strong> {rule.isActive ? 'Active' : 'Inactive'}</Typography>
            </Paper>
            
            <Paper sx={{ p: 3, mb: 3 }}>
              <Typography variant="subtitle1" gutterBottom>Trigger</Typography>
              <Typography><strong>Type:</strong> {rule.triggerType}</Typography>
              {rule.triggerType === 'event' && (
                <Typography><strong>Event Type:</strong> {rule.triggerConfig.eventType}</Typography>
              )}
              {rule.triggerType === 'schedule' && (
                <Typography><strong>Schedule:</strong> {rule.triggerConfig.schedule}</Typography>
              )}
              {rule.triggerType === 'condition' && (
                <Box>
                  <Typography><strong>Entity:</strong> {rule.triggerConfig.condition.entity}</Typography>
                  <Typography><strong>Field:</strong> {rule.triggerConfig.condition.field}</Typography>
                  <Typography><strong>Operator:</strong> {rule.triggerConfig.condition.operator}</Typography>
                  <Typography><strong>Value:</strong> {rule.triggerConfig.condition.value}</Typography>
                </Box>
              )}
            </Paper>
            
            <Paper sx={{ p: 3 }}>
              <Typography variant="subtitle1" gutterBottom>Actions ({rule.actions.length})</Typography>
              {rule.actions.map((action, index) => (
                <Box key={index} sx={{ mb: 2 }}>
                  <Typography><strong>Action {index + 1}:</strong> {action.actionType}</Typography>
                </Box>
              ))}
            </Paper>
          </Box>
