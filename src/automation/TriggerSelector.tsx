...
const TriggerSelector: React.FC<TriggerSelectorProps> = ({
  ...
  return (
    <Box>
      ...
      >
        <FormControlLabel value="event" control={<Radio />} label="Event-based" />
        <FormControlLabel value="condition" control={<Radio />} label="Condition-based" />
        <FormControlLabel value="schedule" control={<Radio />} label="Schedule-based" />
      </RadioGroup>
      ...
      {triggerType === 'condition' && (
        <Box>
          ...
          <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <FormControl fullWidth error={!!validationErrors.conditionEntity}>
                  ...
                  {validationErrors.conditionEntity && (
                    <FormHelperText>{validationErrors.conditionEntity}</FormHelperText>
                  )}
                </FormControl>
              </Grid>
                ...
                >
                  ...
                  >
                    {triggerConfig.condition?.entity && 
                      entityFields[triggerConfig.condition.entity]?.map((field) => (
                  ...
                  {validationErrors.conditionField && (
                    <FormHelperText>{validationErrors.conditionField}</FormHelperText>
                  )}
                </FormControl>
              </Grid>
              ...
              <Grid item xs={12} md={6}>
                ...
                {validationErrors.conditionValue && (
                  <FormHelperText error>{validationErrors.conditionValue}</FormHelperText>
                )}
              </Grid>
            </Grid>
          </Paper>
        ...
        <Box>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <TextField
                ...
                helperText={validationErrors.schedule || "Use cron syntax (e.g., '0 9 * * 1' for every Monday at 9 AM)"}
              />
            </Grid>
          </Grid>
          ...
          <Box sx={{ mt: 2, display: 'flex', alignItems: 'center' }}>
            ...
            <Tooltip title="Every day at midnight">
              <Typography variant="body2" sx={{ ml: 1, cursor: 'pointer' }} onClick={() => onTriggerConfigChange({ schedule: '0 0 * * *' })}>
                Daily
              </Typography>
            </Tooltip>
            <Typography variant="body2" sx={{ mx: 0.5 }}>|</Typography>
            <Tooltip title="Every Monday at 9 AM">
              <Typography variant="body2" sx={{ cursor: 'pointer' }} onClick={() => onTriggerConfigChange({ schedule: '0 9 * * 1' })}>
                Weekly
              </Typography>
            </Tooltip>
            <Typography variant="body2" sx={{ mx: 0.5 }}>|</Typography>
            <Tooltip title="First day of each month at 6 AM">
              <Typography variant="body2" sx={{ cursor: 'pointer' }} onClick={() => onTriggerConfigChange({ schedule: '0 6 1 * *' })}>
                Monthly
              </Typography>
            </Tooltip>
          </Box>
