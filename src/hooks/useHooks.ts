 ...
 */
export const usePermissions = (role, requiredPermissions = []) => {
  // Role-based permission mapping
  const rolePermissions = {
    admin: ['read', 'write', 'delete', 'manage_users', 'manage_settings', 'approve', 'export'],
 ...
 */
export const useOfflineSync = (entityType, syncFunction) => {
  ...
  useEffect(() => {
    const loadPendingChanges = async () => {
      try {
        ...
        const storedChanges = localStorage.getItem(`${entityType}_pending_changes`);
        if (storedChanges) {
  ...
  const addPendingChange = (change) => {
    const updatedChanges = [...pendingChanges, { ...change, id: Date.now().toString() }];
    setPendingChanges(updatedChanges);
    localStorage.setItem(`${entityType}_pending_changes`, JSON.stringify(updatedChanges));
  };
